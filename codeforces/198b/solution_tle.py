...
class Solution(object):
    ...
    def spfa(self, n, flatten_n, k, edges, e_next, e_target, is_game_point):
        ...
        start_node = 0
        dist[start_node] = 0
        nodes_to_relax = Queue.Queue(maxsize=flatten_n)
        nodes_to_relax.put(start_node)
        in_nodes_to_relax = set([start_node])

        while not nodes_to_relax.empty():
            current = nodes_to_relax.get()
